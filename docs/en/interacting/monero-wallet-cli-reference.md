...
| Option                         | Description
|--------------------------------|--------------------------------------------------------------------------------------------------------------------------------------
| `--subaddress-lookahead <arg>` | **Note: This flag can only be set during wallet creation or restore**<br>You may modify the lookahead while logged into the wallet by using<br>`set subaddress-lookahead m:n`<br>Accepts `m:n`, by default `50:200`. The first value is the number of accounts and the second value is the number of subaddresses per account. <br><br>The wallet will not check for payments to subaddresses further than `n-1` away from the last received payment. This can happen if you generated unique `n` subaddresses in a row but none of them received a transfer.<br><br>On the other hand the more subaddresses you set to look ahead, the longer it takes to create your wallet, because they must be pre-computed. This is normally not a concern, except for hardware wallets. On the Ledger the default value of `50:200` can take over 20 minutes (one time on wallet creation)!
...
| `--tx-notify <arg>`          | Run a program for each new incoming transaction, '%s' will be replaced by the transaction hash. Should be used in conjunction with `monero-wallet-rpc`.
